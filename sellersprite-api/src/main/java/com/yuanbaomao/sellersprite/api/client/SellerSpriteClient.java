...
public class SellerSpriteClient {
    ...
    public <T> T get(String path, ParameterizedTypeReference<SellerSpriteResponse<T>> responseType) {
        ...
        return execute(HttpMethod.GET, path, requestId, () -> restClient.get()
                .uri(path)
    ...
    public <T> T postMultipart(SellerSpriteOperation operation, MultiValueMap<String, Object> parts,
        ...
        requireMethod(operation, HttpMethod.POST);
        AtomicReference<String> requestId = new AtomicReference<>();
        return execute(HttpMethod.POST, operation.getPath(), requestId, () -> restClient.post()
    ...
    private <T> T execute(HttpMethod method, String path, AtomicReference<String> requestId,
            Supplier<SellerSpriteResponse<T>> request) {
        ...
        try {
            ...
            log.debug("SellerSprite 请求完成 requestId={}, method={}, path={}, elapsedMs={}",
                    requestId.get(), method, path, elapsedMillis(startedAt));
            return response.getData();
        } catch (SellerSpriteApiException exception) {
            throw exception;
        } catch (RestClientResponseException exception) {
            log.warn("SellerSprite HTTP 请求失败 requestId={}, method={}, path={}, status={}, elapsedMs={}",
                    requestId.get(), method, path, exception.getStatusCode(), elapsedMillis(startedAt));
