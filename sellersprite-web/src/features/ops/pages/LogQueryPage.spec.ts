...
describe('LogQueryPage', () => {
  it('loads tabs on demand and renders server-masked operation detail', async () => {
    vi.mocked(logApi.pageLoginLogs).mockResolvedValue({
      ...
      records: [{ loginLogId: 'login-1', userId: 'user-1', username: 'admin', loginType: 'LOGIN', success: 1, errorCode: '', failureReason: '', loginIp: '127.0.0.1', loginLocation: '', userAgent: 'browser', deviceName: '', clientType: 'WEB', traceId: 'trace-login', createdAt: 1 }],
    })
    vi.mocked(logApi.pageOperationLogs).mockResolvedValue({
      current: 1, size: 20, total: 1,
      records: [{ operationLogId: 'operation-1', userId: 'user-1', username: 'admin', moduleName: 'PermissionController', operationName: '同步菜单接口绑定', operationType: 'UPDATE', httpMethod: 'PUT', requestUri: '/api/permissions/functions/api-bindings/sync', requestParams: '', responsePayload: '', responseStatus: 200, success: 1, errorMessage: '', clientIp: '127.0.0.1', userAgent: 'browser', costMs: 12, traceId: 'trace-operation', createdAt: 2 }],
    })
    vi.mocked(logApi.getOperationLog).mockResolvedValue({
      operationLogId: 'operation-1', userId: 'user-1', username: 'admin', moduleName: 'PermissionController', operationName: '同步菜单接口绑定', operationType: 'UPDATE', httpMethod: 'PUT', requestUri: '/api/permissions/functions/api-bindings/sync', requestParams: '{"authorization":"[REDACTED]"}', responsePayload: '{"code":"00000"}', responseStatus: 200, success: 1, errorMessage: '', clientIp: '127.0.0.1', userAgent: 'browser', costMs: 12, traceId: 'trace-operation', createdAt: 2,
    })
  ...
  it('treats the backend SUCCESS prompt status as successful', async () => {
    ...
    vi.mocked(logApi.pageAiPromptLogs).mockResolvedValue({
      current: 1, size: 20, total: 1,
      records: [{ promptRecordId: 'prompt-1', conversationId: 'conversation-1', userId: 'user-1', provider: 'openai', model: 'gpt-5.5', requestMessages: '', promptSummary: '', promptTruncated: 0, responseContent: '', responseMetadata: '', promptTokens: 10, completionTokens: 20, totalTokens: 30, finishReason: 'STOP', status: 'SUCCESS', errorType: '', errorMessage: '', costMs: 100, traceId: 'trace-ai', createdAt: 3 }],
    })
    const wrapper = mount(LogQueryPage, { global: { stubs: { Teleport: true } } })
    await flushPromises()
