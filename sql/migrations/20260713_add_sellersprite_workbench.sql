USE `sellersprite_service`;

-- 本迁移只补充卖家精灵工作台、SUPER 管理员授权和接口绑定，不修改任何用户密码。
START TRANSACTION;

SET @workbench_now_ms = CAST(UNIX_TIMESTAMP(CURRENT_TIMESTAMP(3)) * 1000 AS UNSIGNED);
SET @workbench_admin_user_id = (
  SELECT `user_id`
  FROM `user`
  WHERE `username` = 'admin' AND `deleted` = 0
  ORDER BY `created_at`
  LIMIT 1
);

INSERT INTO `sys_function` (
  `sys_function_id`, `parent_id`, `function_code`, `function_name`, `function_type`,
  `route_path`, `component_path`, `permission_code`, `icon`, `visible`, `cacheable`,
  `external_link`, `sort_order`, `status`, `created_at`, `updated_at`,
  `created_by`, `updated_by`, `deleted`, `remark`
) VALUES (
  '019f4d77-0000-7000-8000-000000000012', '0', 'sellersprite.workbench', '卖家精灵工作台', 'MENU',
  '/sellersprite/workbench', 'sellersprite/workbench', 'sellersprite:workbench:view', 'DataBoard', 1, 0,
  NULL, 25, 1, @workbench_now_ms, @workbench_now_ms,
  COALESCE(@workbench_admin_user_id, ''), COALESCE(@workbench_admin_user_id, ''), 0,
  '系统初始化卖家精灵工作台菜单'
)
ON DUPLICATE KEY UPDATE
  `parent_id` = VALUES(`parent_id`),
  `function_name` = VALUES(`function_name`),
  `function_type` = VALUES(`function_type`),
  `route_path` = VALUES(`route_path`),
  `component_path` = VALUES(`component_path`),
  `permission_code` = VALUES(`permission_code`),
  `icon` = VALUES(`icon`),
  `visible` = VALUES(`visible`),
  `cacheable` = VALUES(`cacheable`),
  `external_link` = VALUES(`external_link`),
  `sort_order` = VALUES(`sort_order`),
  `status` = VALUES(`status`),
  `updated_at` = @workbench_now_ms,
  `updated_by` = COALESCE(@workbench_admin_user_id, ''),
  `deleted` = 0,
  `remark` = VALUES(`remark`);

SET @workbench_function_id = (
  SELECT `sys_function_id`
  FROM `sys_function`
  WHERE `function_code` = 'sellersprite.workbench' AND `deleted` = 0
  ORDER BY `created_at`
  LIMIT 1
);

-- role_code=admin 是当前权限拦截器识别的 SUPER 管理员角色。
INSERT INTO `role_function` (
  `role_function_id`, `role_id`, `sys_function_id`, `created_at`, `updated_at`,
  `created_by`, `updated_by`, `deleted`, `remark`
)
SELECT
  '019f4d77-0000-7000-8000-000000000112', `role`.`role_id`, @workbench_function_id,
  @workbench_now_ms, @workbench_now_ms,
  COALESCE(@workbench_admin_user_id, ''), COALESCE(@workbench_admin_user_id, ''), 0,
  '系统初始化 SUPER 管理员卖家精灵工作台授权'
FROM `role` AS `role`
WHERE `role`.`role_code` = 'admin'
  AND `role`.`status` = 1
  AND `role`.`deleted` = 0
  AND @workbench_function_id IS NOT NULL
ON DUPLICATE KEY UPDATE
  `role_id` = VALUES(`role_id`),
  `sys_function_id` = VALUES(`sys_function_id`),
  `updated_at` = @workbench_now_ms,
  `updated_by` = COALESCE(@workbench_admin_user_id, ''),
  `deleted` = 0,
  `remark` = VALUES(`remark`);

-- 接口目录通常在应用启动后同步；尚无 SellerSprite 接口时本语句写入 0 行且不会失败。
-- 完成接口目录同步后可安全重跑本迁移，将全部启用的 /api/sellersprite/** 接口关联到工作台。
INSERT INTO `function_api` (
  `function_api_id`, `sys_function_id`, `sys_api_id`, `created_at`, `updated_at`,
  `created_by`, `updated_by`, `deleted`, `remark`
)
SELECT
  CONCAT('019f4d77-0000-7000-8001-', SUBSTRING(MD5(`api`.`sys_api_id`), 1, 12)),
  @workbench_function_id, `api`.`sys_api_id`, @workbench_now_ms, @workbench_now_ms,
  COALESCE(@workbench_admin_user_id, ''), COALESCE(@workbench_admin_user_id, ''), 0,
  '系统初始化卖家精灵工作台接口绑定'
FROM `sys_api` AS `api`
WHERE `api`.`path_pattern` LIKE '/api/sellersprite/%'
  AND `api`.`status` = 1
  AND `api`.`deleted` = 0
  AND @workbench_function_id IS NOT NULL
ON DUPLICATE KEY UPDATE
  `updated_at` = @workbench_now_ms,
  `updated_by` = COALESCE(@workbench_admin_user_id, ''),
  `deleted` = 0,
  `remark` = VALUES(`remark`);

COMMIT;
